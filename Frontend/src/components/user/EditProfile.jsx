...
const EditProfile = () => {
    ...
    const dispacth = useDispatch(); // Note: This should typically be spelled 'dispatch'
    ...
    onSubmit: ({ value }) => {
        ...
        dispacth(updateUser(updatedFields)); // Note: 'dispacth' should be 'dispatch'
        navigate("/profile");
        toast.success("Profile Updated")
    },
});
        useEffect(() => {
        // Check for errors to display and clear them
        if (errors && errors.length > 0) {
            toast.error(errors);
            dispacth(userActions.clearErrors()); // Note: 'dispacth' should be 'dispatch'
        } 
        // If user data is available, set the form fields
        else if (user) {
            form.setFieldValue("name", user.name);
            form.setFieldValue("phoneNumber", user.phoneNumber);
            // Set avatar field value, using placeholder if URL is missing
            form.setFieldValue("avatar", user.avatar.url || "https://i.pravatar.cc/150?img3");
        }
    }, [user,errors,form,dispacth]); // Re-run effect when the user object changes
